...
def is_user_authorized(user_id):
    ...
    if info['expiry_date'] < datetime.datetime.now():
        ...
        bot.sendMessage(user_id, 'Your plan has expired. Contact @SUKALEBOK06 to renew.')
        return False
    return True
...
def handle_message(msg):
    ...
    if not is_user_authorized(user_id):
        bot.sendMessage(chat_id, f'You dont have plan. Contact @SUKALEBOK06 for buying plan.\n\nYour ID: <code>{user_id}</code>', parse_mode='HTML')
        return
...
- /listblacklist - <strong>See all BlackList.</strong>'''
            ...
            message = '<strong>Owner</strong>: @SUKALEBOK06\n<strong>Version</strong>: 2.0\nIf you want to buy source. Contact @SUKALEBOK06'
            bot.sendMessage(chat_id, message, parse_mode='HTML')
            ...
            if not is_user_authorized(user_id):
                bot.sendMessage(chat_id, 'You dont have plan. Contact @SUKALEBOK06 for buying plan.')
                return
            ...
            keyboard = ReplyKeyboardMarkup(
                keyboard=[
                    [KeyboardButton(text='TLS-SILIT'), KeyboardButton(text='TLS-NARUTO'), KeyboardButton(text='RAPID-SILIT'), KeyboardButton(text='CRASH-SILIT'), KeyboardButton(text='SILITBYPASS')],
                    [KeyboardButton(text='❌ Cancel')],
                ],
                resize_keyboard=True,
                one_time_keyboard=True
          ...
          states[chat_id] = user_state
            ...
            if not user_state.target:
              ...
              user_attack_slots = info.get('attack_slots', 0)
              if concurrents <= 0:
                bot.sendMessage(chat_id, 'Your concurrents count is zero, Contact @SUKALEBOK06 To Buy Plan')
                del states[chat_id]  
                return
        ...
        elif text == 'BYPASS' or text == 'TLSOP' or text == 'SPIKE' or text == 'VIP-TLS' or text == 'SUPER' or text == 'HTTPS':
            ...
            user_state = UserState(method=method)
            states[chat_id] = user_state
        elif chat_id in states and states[chat_id].method in ['BYPASS', 'TLSOP', 'SPIKE', 'VIP-TLS', 'SUPER', 'HTTPS']:
...
if __name__ == '__main__':
    ...
    admin_users = set()
    admin_ids = [5713258194]  # admin user id here
